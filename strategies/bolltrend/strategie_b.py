    ...
    try:
        ...
        for pos in positions_data:
            if pos.pair in df_list and pos.side == "long":
                pct_exposition = (float(pos.size) * float(pos.current_price)) / usd_balance
                positions_exposition[pos.pair]["long"] += pct_exposition
                long_exposition += pct_exposition
            elif pos.pair in df_list and pos.side == "short":
                pct_exposition = (float(pos.size) * float(pos.current_price)) / usd_balance
                positions_exposition[pos.pair]["short"] += pct_exposition
                ...
                try:
                    ...
                    if open_long(row) and "long" in type:
                        ...
                        if temp_var > max_var or temp_long_exposition > max_side_exposition:
                            ...
                            if production:
                                await exchange.place_order(pair, "buy", long_quantity, size=exchange.amount_to_precision(pair, long_quantity_in_usd), reduce=False)
                        ...
                        if temp_var > max_var or temp_short_exposition > max_side_exposition:
                            ...
                            if production:
                                await exchange.place_order(pair, "sell", short_quantity,size=exchange.amount_to_precision(pair, short_quantity_in_usd), reduce=False)
